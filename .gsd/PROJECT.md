...
M002 is complete (all 4 slices done). S01 (Docker Production Container): multi-stage Dockerfile, `output: 'standalone'`, SQLite volume, `restart: unless-stopped`, R030 validated. S02 (PWA Shell & Home Screen): real 192×192 and 512×512 PNG icons (🧊 dark theme); `@serwist/next` wraps `next.config.ts`; `app/sw.ts` with precache manifest, `skipWaiting`, `clientsClaim`, offline fallback; R023 and R024 validated. S03 (Last-Used Fridge Memory): `LastFridgeWriter` island writes `localStorage['lastFridgeId']` on each fridge visit; `LastFridgeRedirect` at root redirects to last fridge; R025 and R026 validated. S04 (mDNS Hostname): `bonjour-service@1.3.0` installed; `lib/mdns/advertise.ts` publishes `thefridge` on port 3000 with SIGTERM/SIGINT cleanup; `instrumentation.ts` starts advertisement in nodejs+production only; `outputFileTracingIncludes` forces package into standalone; `docker-compose.host.yml` uses `network_mode: host` for mDNS multicast on Linux; `bash scripts/verify-s04-mdns.sh` passes 4/4 cross-platform checks (2 Linux-only hostname checks pending on home device); R031 validated. Final integrated acceptance (docker compose up on home Linux device + phone PWA install + last-used fridge + `thefridge.local` resolution) is the remaining human UAT step.
...
- [x] M002: Zero-friction access & deployment — Docker one-command setup, PWA home screen installation, last-used fridge memory, and mDNS hostname for any home device
